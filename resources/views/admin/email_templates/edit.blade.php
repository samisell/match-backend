        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="flex justify-between items-center py-4 px-6 bg-white border-b-4 border-wine-dark">
                <div class="flex items-center">
                    <h2 class="text-2xl font-semibold text-gray-800">Edit Email Template: {{ $emailTemplate->name }}</h2>
                </div>
                <div class="flex items-center">
                    ...
                    <div class="bg-white shadow-md rounded my-6 p-6">
                        <form action="{{ route('admin.email_templates.update', $emailTemplate->id) }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $emailTemplate->name) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @error('name')
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="subject" id="subject" value="{{ old('subject', $emailTemplate->subject) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @error('subject')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="body" id="body" rows="10" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>{{ old('body', $emailTemplate->body) }}</textarea>
                                @error('body')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="type" id="type" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                    <option value="user" {{ old('type', $emailTemplate->type) == 'user' ? 'selected' : '' }}>User Notification</option>
                                    <option value="admin" {{ old('type', $emailTemplate->type) == 'admin' ? 'selected' : '' }}>Admin Notification</option>
                                </select>
                                @error('type')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
